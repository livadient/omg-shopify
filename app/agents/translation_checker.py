...
async def check_and_fix_translations() -> dict:
    ...
    try:
        ...
        await send_error_email("Hermes", e)
        raise
...
async def _send_report_email(results: list[dict], total_registered: int, error_count: int) -> None:
    ...
    <div style="font-family:sans-serif;max-width:900px;margin:0 auto;">
        <div style="background:#2563eb;color:white;padding:20px;border-radius:8px 8px 0 0;">
            <h2 style="margin:0;">Hermes here — translation run complete</h2>
            <p style="margin:4px 0 0;opacity:0.9;">I translated {total_registered} fields tonight{f' (hit {error_count} snags)' if error_count else ' — all clean!'}</p>
        </div>
    ...
    await send_agent_email(
        subject=f"[Hermes] {total_registered} fields translated{f', {error_count} errors' if error_count else ' — all good!'}",
        html_body=html,
    )
